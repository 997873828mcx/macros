...
class MainWindow(QMainWindow):
    ...
    def create_toolbars(self):
        ...
        self.event_model = QStandardItemModel(self.event_combo)
        self.event_combo.setModel(self.event_model)
        self.event_model.itemChanged.connect(lambda _: self.update_display())
    ...
    def create_display_tab(self):
        ...
        # Intersections and hits checkboxes
        self.clusters_checkbox_tpc = QCheckBox("Intersections")
        ...
        self.hits_checkbox_tpc = QCheckBox("G4 Hits")
        ...
        threshold_layout.addRow("Intersection value ≥", self.cluster_adc_spinbox)
        ...
        threshold_layout.addRow("G4 hit value ≥", self.hit_adc_spinbox)
    ...
    def load_data(self):
        ...
        for filename in filenames:
            ...
            events_in_file = self._extract_events_from_polydata(cluster_polydata)
            events_in_file.update(self._extract_events_from_polydata(hit_polydata))
            if events_in_file:
                self.available_events.update(events_in_file)
    ...
    def refresh_event_combo(self):
        if not hasattr(self, "event_model"):
            return
        checked_events = set()
        for row in range(self.event_model.rowCount()):
            item = self.event_model.item(row)
            if item is None:
                continue
            if item.checkState() == Qt.Checked:
                try:
                    checked_events.add(int(item.text().split()[1]))
                except (ValueError, IndexError):
                    continue

        self.event_model.blockSignals(True)
        self.event_model.clear()
        for event_id in sorted(self.available_events):
            item = QStandardItem(f"Event {event_id}")
            item.setFlags(Qt.ItemIsEnabled | Qt.ItemIsUserCheckable)
            if checked_events:
                state = Qt.Checked if event_id in checked_events else Qt.Unchecked
            else:
                state = Qt.Checked
            item.setData(state, Qt.CheckStateRole)
            self.event_model.appendRow(item)
        self.event_model.blockSignals(False)

    @staticmethod
    def _extract_events_from_polydata(polydata):
        if polydata is None or polydata.n_points == 0:
            return set()
        events = polydata.point_data.get("event")
        if events is None:
            return set()
        try:
            ...
            return set()
        event_ids = set()
        for value in np.unique(event_array):
            try:
