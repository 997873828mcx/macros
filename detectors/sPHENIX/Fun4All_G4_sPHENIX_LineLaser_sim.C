...
// Force‑load local builds of libraries needed by DirectLaser and DNL, so Condor
// runs pick up the patched versions instead of release libs on worker nodes.
// Safe to call even if already loaded; adjust path if your local install differs.
gSystem->Load("/sphenix/user/dcxchenxi/install/lib/libg4tpc.so");
gSystem->Load("/sphenix/user/dcxchenxi/install/lib/libtpccalib.so");


// bring TpcLaserDNL class declaration into scope for the interpreter (use repo-local header)
#include "/sphenix/user/dcxchenxi/develope/coresoftware/offline/packages/tpccalib/TpcLaserDNL.h"

// For HepMC Hijing
// try inputFile = /sphenix/sim/sim01/sphnxpro/sHijing_HepMC/sHijing_0-12fm.dat

int Fun4All_G4_sPHENIX_LineLaser_sim(
    const int nEvents = 1,
...
//  G4MAGNET::magfield_rescale = 1.;  // make consistent with expected Babar field strength of 1.4T
  ...
  // (moved) TPC_Cells will be called after registering DNL to ensure
  // DNL runs after drift in per-event processing (module order)

  // Ensure Acts geometry node exists before DNL InitRun
  // Align ActsGeometry drift velocity (used by TpcLaserDNL) with the
  // mixture-specific value used in TPC_Cells
  {
    ...
    // ActsGeometry picks this up in ACTSGEOM::ActsGeomInit()
    G4TPC::tpc_drift_velocity_reco = drift_vel;
  }
  ...
  dnl->set_max_dz(1.0e6);   // cm; effectively disable z-cut (acts vs sim vdrift mismatch)
  // derive DNL output name from macro outputFile argument
  {
    std::string dnlout = outputFile;
    const std::string ext = ".root";
    const auto pos = dnlout.find(ext);
    if (pos != std::string::npos) dnlout.erase(pos, ext.length());
    dnlout += "_laser_dnl.root";
    dnl->set_outputfile(dnlout);
  }
  se->registerSubsystem(dnl);

  // Now enable direct-laser and run TPC cell/drift after DNL is registered.
  // In Fun4All, later-registered modules typically process earlier per event.
  // Registering drift after DNL ensures drift runs before DNL.
 

  // For direct DNL studies, skip downstream clustering, tracking, and QA
  ...
  if (Enable::TPC_CLUSTER)
    {
      // direct-laser study: disable central membrane clustering (it requires LaserEventInfo)
