...
campaign_tag=${CAMPAIGN_TAG:-tpc_pattern_trackpoints}
...
export LD_LIBRARY_PATH=/sphenix/user/mitrankova/F4A/TPC_pattern_reco/install/lib:/sphenix/user/dcxchenxi/install/lib:${LD_LIBRARY_PATH:-}
...
event_start=$((file_start * events_per_input_file))
...
outroot="${job_outdir}/tpc_pattern_trackpoints_${padded_id}_gskip${event_start}_nev${nevents}.root"

echo "Running TPC pattern-reco track-point dumper:"
...
echo "  event_start=${event_start}"
echo "  nevents=${nevents}"
echo "  out=${outroot}"

root.exe -l -b -q "Fun4All_TpcPatternTrackPoints.C(${nevents}, \"${chunk_list}\", \"${outroot}\")"
