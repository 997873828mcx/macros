...
campaign_tag=${CAMPAIGN_TAG:-raw_finaltrack_pt}
...
export LD_LIBRARY_PATH=/sphenix/user/mitrankova/F4A/TPC_pattern_reco/install/lib:${LD_LIBRARY_PATH:-}
...
outroot="${job_outdir}/raw_finaltrack_pt_${padded_id}_fskip${file_start}_nfiles${nfiles}.root"

echo "Running raw FINALTRACKS pT dump:"
...
root.exe -l -b -q "Fun4All_RawFinalTrackPt.C(${nevents}, \"${chunk_list}\", \"${outroot}\", \"${node_name}\", ${pt_min}, ${pt_max}, ${pt_bins})"
