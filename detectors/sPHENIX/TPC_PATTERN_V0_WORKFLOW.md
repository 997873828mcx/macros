...
In the intended post-integration layout, the runtime libraries are
`libtpctrackreco.so` and `libTrackingDiagnostics.so`; the ROOT macro explicitly
loads both. The worker selects release `new.15` and then overlays the local
installation at:

```text
/sphenix/user/dcxchenxi/install
```

The dynamic loader uses an overlay library when present and otherwise can fall
back to the selected release. Older development installations placed these
classes in `libg4tpc.so`; do not mix headers from the new package layout with
incompatible legacy binaries. Do not rebuild the active installation or edit
the runtime macro while a production campaign is in flight unless the change
is intentionally meant for those jobs.
...
For an ordinary merged file:

```bash
root -l -b -q '/sphenix/user/dcxchenxi/develope/macros/TrackingProduction/drawAP.C("output/tpc_pattern_v0_my_sample_kalman_v1_merged.root",true,"output/AP_my_sample_kalman_v1")'
```

The plotting macro can apply another selection. Remember that `pairTree` has
already been reduced by all cuts enabled during reconstruction; plotting cannot
recover pairs that were not written. For exploratory studies, use loose
campaign cuts and apply tighter analysis cuts afterward.
