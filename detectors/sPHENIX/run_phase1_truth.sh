...
campaign_tag=${CAMPAIGN_TAG:-phase1_truth}
...
event_out="${job_outdir}/phase1_truth_events_${padded_id}_fskip${file_start}_nfiles${nfiles}.root"
v0_out="${job_outdir}/phase1_truth_v0_decays_${padded_id}_fskip${file_start}_nfiles${nfiles}.root"

echo "Running Phase 1 truth QA trees:"
...
echo "  event_out=${event_out}"
echo "  v0_out=${v0_out}"

root.exe -l -b -q "Fun4All_Phase1Truth.C(${nevents}, \"${chunk_list}\", \"${event_out}\", \"${v0_out}\")"

for outroot in "${event_out}" "${v0_out}"; do
  if [[ -f "${outroot}" ]]; then
